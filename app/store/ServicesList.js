	...
	,data: [
		{
			...
			,type: 'messaging'
			,js_unread: `let checkUnread=()=>{
	const elements=document.querySelectorAll("._8nE1Y > ._2KKXC > .Dvjym > span > ._1pJ9J > .l7jjieqr");
	let count=0;
	let b=0;
	for(const i of elements){
		const gp=i.parentNode.parentNode;
		0===gp.querySelectorAll('#pane-side *[data-icon="muted"]').length&&count++
		const iTrim=parseInt(i.textContent.trim());
		iTrim%1==0&&(b+=iTrim)
	}
	hamsket.updateBadge(b);
	console.log(b);
};
...
return unregister_queue}).then(queue=>{}).catch(err=>{});`
		},
		{
			 id: 'slack'
			,logo: 'slack.png'
		...
		},
		{
			...
			,url: '___'
			,type: 'messaging'
			,js_unread: `let checkUnread=()=>{let b=0;const badges=document.querySelectorAll(".has-unread:not(.is-muted) > .badge");for(let i of badges){const iTrim=parseInt(i.textContent.trim());iTrim%1==0&&(b+=iTrim)}hamsket.updateBadge(b);console.log(b);};setInterval(checkUnread,1e3);`
		},
		{
			 id: 'linkedin'
			,logo: 'linkedin.png'
